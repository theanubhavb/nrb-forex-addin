...
// 
// NOTE: Temporarily comment out this entire block if the function still fails,
//       as it uses Excel JS APIs that can sometimes cause issues during initialization.
// =====================================================================================
...
    try {
        // **FIX: Removed custom 'User-Agent' header**
...
            // Correctly handling Excel Serial Number
...
            // Attempting reliable ISO 8601 parsing (YYYY-MM-DD)
            let isoMatch = str.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
            if (isoMatch) {
                const [_, y, m, d] = isoMatch;
                dateObj = new Date(`${y}-${m.padStart(2, '0')}-${d.padStart(2, '0')}`);
            } else if (/^(\d{1,2})-(\d{1,2})-(\d{4})$/.test(str)) {
                // Assuming DD-MM-YYYY or MM-DD-YYYY and converting to YYYY-MM-DD
                const [_, d, m, y] = str.match(/^(\d{1,2})-(\d{1,2})-(\d{4})$/)!;
                dateObj = new Date(`${y}-${m.padStart(2, '0')}-${d.padStart(2, '0')}`);
            } else {
                // FALLBACK - most likely to fail on Mac
